...
class admm_op():
    ...
    def __init__ (self, model, ema, b, admm_iter=10, rho=1e-4, mu=10, tau_incr=1.5, tau_decr=1.5):
        ...
        for key, value in ema.ema.named_parameters():
            if key in quantdict:
                print(key)
                self.emaW.append(value)
                self.storeemaW.append(value.clone())
    ...
    def update(self, epoch):
        ...
        for i in range(len(self.W)):
            ...
            Qi = Vi/self.a[i]
            Qi = torch.round((Qi-1)/2)*2+1  #TODO might be a problem
    ...
    def apply_quantval(self):
        ...
        for key, value in self.model.named_parameters():
            if key in quantdict:
                ...
                self.ema.ema.state_dict()[key].copy_(self.Z[i])
                i = i + 1
    ...
    def applyquantW(self):
        for i in range(len(self.W)):
            ...
            self.storeemaW[i] = self.emaW[i].clone()
            self.emaW[i].data.copy_(self.Z[i])

    def restoreW(self):
        for i in range(len(self.W)):
            self.W[i].data.copy_(self.storeW[i])

            self.emaW[i].data.copy_(self.storeemaW[i])
...
def weightsdistribute(model):
    for key, value in model.named_parameters():
        if key in quantdict:
            ...
            print(unique,":", unique.shape, ":", count)
